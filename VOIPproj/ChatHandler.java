...
public class ChatHandler extends Thread {
    ...
    // TODO: add more code to this thread to stop the inner while loop or it will
    // wait forever?
    ...
    /**
     * This thread handles processing of received items that were in the received
     * byte vector
     */
    public class ReceivedQueueProcessorThread extends Thread {
        ...
        private void processData(int sizeread, byte[] bytepassedObj) {
            synchronized (handlers) {
                ...
                // TODO: Checking for size < 100 is not good check of commands / text since
                // messages
                ...
                if ((sizeread > 2 && sizeread < 100 && passedObj.length() >= 2
                    ...
                    for (int i = 0; i < handlers.size(); i++) {
                        // if someone is trying to log in with a nick that is already used reject their
                        // log in
                        ChatHandler tmp = (ChatHandler) (handlers.elementAt(i)); // this can be used for password
                    ...
                    if (IAmTalking == true) {
                        ...
                        ImTalkingCounter = 0;// reset the talk counter so next time someone taks it could notify them
                                             // that someone is talking
                        IAmTalking = false;
                    }
                    // notify other people that nobody is talking
                } else if (sizeread >= 2 && sizeread < 100 && passedObj.length() >= 2
                        && (passedObj.substring(0, 2).equals("PR"))) {
                    // packet recieved send next one
                    lastpacketrecieved = true;
                    // Mute one of the users
                } else if (sizeread > 2 && sizeread < 100 && passedObj.length() > 4
                        && passedObj.substring(0, 4).equals("MUTE")) {
                    if (IAmAdmin) {
                        for (int i = 0; i < handlers.size(); i++) {
                            ;
                            ChatHandler tmp = (ChatHandler) (handlers.elementAt(i));
                            if (tmp.nick.equals(passedObj.substring(4, passedObj.length() - 5))) {
                                ...
                                if (tmp.IAmTalking == true) { // tell everyone that they can now talk
                                    for (int j = 0; j < handlers.size(); j++) {
                                        ...
                                        tmp2.cs.writebyte(
                                                ("NT" + ptrtoThis.nick + MultiChatConstants.BREAKER).getBytes());
                                        // tmp2.out.write( ("NT" + this.nick).getBytes() );
                                        // tmp2.out.flush();

                                    }
                                    ImTalkingCounter = 0;// reset the talk counter so next time someone taks it could
                                                         // notify them that someone is talking
                ...
                } else if (sizeread > 2 && sizeread < 100 && passedObj.length() > 6
                        && passedObj.substring(0, 6).equals("UNMUTE")) {
                    if (IAmAdmin) {
                        for (int i = 0; i < handlers.size(); i++) {
                            ;
                            ChatHandler tmp = (ChatHandler) (handlers.elementAt(i));
                    ...
                    // text talking or low audio size (SW: Probably not low audi of starts with
                    // TXT?)
                    ...
                    if ((handlers.size() <= 2 || someoneIsTalking == false
                            || IAmTalking == true && ptrtoThis.nick != "")
                            && (IAmMute == false)) {
    ...
    public void run() {
        ...
        try {
            ...
            while (keepGoing) {
                ...
                // recievedByteVector.addElement( ((Object)(new vectorandsize( bytepassedObj,
                // sizeread ))) );
